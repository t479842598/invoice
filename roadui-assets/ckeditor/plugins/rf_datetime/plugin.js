        ...
        CKEDITOR.dialog.add(pluginName, function (editor) {
            return {
                        ...
                        [
                            {
                                id: pluginName + '_attr',
                                type: 'html',
                                html: '<iframe style="width:100%;height:480px;border:0;" frameborder="0" src="index.html?loadurl=' + encodeURIComponent('/form/plugins/datetime.vue?editorname=' + editor.name) + editor.iframeQueryString + '"></iframe>'
                            }
                        ]
                }],
                onShow: function () {
                    if (editor.initFunction_datetime) {
                        editor.initFunction_datetime(editor.currentSelectEditorElement);
                    }
                },
                onHide: function () {
                ...
                onOk: function () {
                    ...
                    input.setAttribute("value", (json.label || '日期时间') + (bindField.length === 0 ? "" : "(" + bindField + ")"));
                    ...
                    input.setAttribute("data-selecttype", json.selectType || 'date');
                    input.setAttribute("data-shortcuts", json.shortcuts || 'false');
                    input.setAttribute("data-clearable", json.clearable || '');
                    input.setAttribute("data-format", json.format || '');
                    input.setAttribute("data-valueformat", json.valueFormat || '');
                    input.setAttribute("data-separator", json.separator || '');
                    input.setAttribute("data-starttime", json.startTime || '');
                    input.setAttribute("data-endtime", json.endTime || '');
                    input.setAttribute("data-mintime", json.minTime || '');
                    input.setAttribute("data-maxtime", json.maxTime || '');
                    input.setAttribute("data-steptime", json.stepTime || '');
