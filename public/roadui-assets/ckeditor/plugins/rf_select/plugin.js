        ...
        CKEDITOR.dialog.add(pluginName, function (editor) {
            return {
                        ...
                        [
                            {
                                id: pluginName + '_attr',
                                type: 'html',
                                html: '<iframe style="width:100%;height:510px;border:0;" frameborder="0" src="index.html?loadurl=' + encodeURIComponent('/form/plugins/selects.vue?editorname=' + editor.name) + editor.iframeQueryString + '"></iframe>'
                            }
                        ]
                }],
                onShow: function () {
                    if (editor.initFunction_select) {
                        editor.initFunction_select(editor.currentSelectEditorElement);
                    }
                },
                onHide: function () {
                ...
                onOk: function () {
                    ...
                    input.setAttribute("value", (json.label || '下拉选择') + (bindField.length === 0 ? "" : "(" + bindField + ")"));
                    ...
                    input.setAttribute("data-multiple", json.multiple ? 'true' : 'false');
                    input.setAttribute("data-linkageid", json.linkageId || '');
                    input.setAttribute("data-linkagedatasource", json.linkageDataSource || '');
                    input.setAttribute("data-linkagetext", editor.utils.encodeURI(json.linkageText || ''));
