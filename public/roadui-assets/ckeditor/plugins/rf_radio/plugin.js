CKEDITOR.plugins.add('rf_radio', {
    requires: ['dialog'],
    init: function (editor) {
        var pluginName = 'rf_radio';
        //给自定义插件注册一个调用命令
        ...
        editor.ui.addButton(pluginName, {
            label: "单选按钮",
            command: pluginName,
            icon: this.path + "radio.png",
            toolbar: 'rf_plugins,20'
        });
            ...
            editor.contextMenu.addListener(function (element) {
                if (element && !element.isReadOnly()) {
                    ...
                    if (name == 'input') {
                        ...
                        if (type == 'radio') {
                            editor.currentSelectEditorElement = element;//保存当前焦点对象 currentSelectEditorElement
                            return {rf_radio: CKEDITOR.TRISTATE_OFF, rf_radio_delete: CKEDITOR.TRISTATE_OFF};
                        }
                    }
                }
            });
        ...
        CKEDITOR.dialog.add(pluginName, function (editor) {
            return {
                        ...
                        [
                            {
                                id: pluginName + '_attr',
                                type: 'html',
                                html: '<iframe style="width:100%;height:470px;border:0;" frameborder="0" src="index.html?loadurl=' + encodeURIComponent('/form/plugins/radio.vue?editorname=' + editor.name) + editor.iframeQueryString + '"></iframe>'
                            }
                        ]
                }],
                onShow: function () {
                    if (editor.initFunction_radio) {
                        editor.initFunction_radio(editor.currentSelectEditorElement);
                    }
                },
                onHide: function () {
                ...
                onOk: function () {
                    ...
                    input.setAttribute("value", (json.label || '单选按钮') + (bindField.length === 0 ? "" : "(" + bindField + ")"));
