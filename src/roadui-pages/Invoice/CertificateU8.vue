        ...
        <el-dialog v-model="Selectpro" title="选择科目" @open="openSelectP" :fullscreen="fullscreen"
            ...
            <el-form :inline="true" :model="formproject">
                ...
                <el-form-item style="margin-left: 20px;">
                    <el-button type="primary" @click="getProjectData(true)">查询</el-button>
                </el-form-item>
            </el-form>
...
const getisignseq = async () => {
    try {
        const res = await ajax.post('Accouch/GetSign', qs.stringify({ databasetype: 'U8' }));
        if (res.success) {
            isignseqlist.value = res.data.U8Sign;
        }
    } catch (error) {
        // 这里可以添加你的错误处理逻辑
    }
...
const createpz = async () => {
    ...
    try {
        const res = await ajax.post('Accouch/CreateU8Vouch', qs.stringify({ isignseq: formData.value.isignseq, pagecount: formData.value.pagecount, csign: formData.value.csign, billdate: formData.value.billdate, invoiceids: formData.value.invoiceids, billcodes: formData.value.billcodes, accvouchJSON: JSON.stringify(tableData.value) }));
        if (res.success) {
...
const getData = async () => {
    loading.value = true;
    try {
        const res = await ajax.post('Accouch/GetAccvouchU8', qs.stringify({ ids: id.value }));
