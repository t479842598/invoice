<template>
    <div>
        ...
        <div class="form-container">
            <el-form ref="formbutton" inline>
                <el-form-item>
                    ...
                    <!-- <el-button type="danger" @click="downLoad()" icon="Download">批量下载</el-button> -->
                </el-form-item>
            ...
            <el-table :data="tableData" v-loading="loading" :stripe="pager.tableStripe" border
                ...
                <el-table-column label="操作" align="center" width="200" fixed="right">
                    <template #default="scope">
                        ...
                        <el-button type="primary" size="small" @click="Copy(scope.row.id)">复制</el-button>
                        <el-button type="danger" size="small"
                            @click="Reddel(scope.row.id, scope.row.billcode)">红冲</el-button>
                        <el-button type="danger" size="small" @click="DeleteAll(scope.row.id, scope.row.billcode)"
                            plain>删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog v-model="RedShow" :fullscreen="fullscreens" style="position: relative;" :title="RedTitle" align-center
            ...
            <RedInvoice v-model:blueid="blueid" v-model:id="id" v-model:readonly="readonly"></RedInvoice>
        ...
        <!-- 复制页面 -->
        <el-dialog v-model="CopydialogShow" :title="CopydialogTitle" @close="Copyfullscreen = false"
            style="position: relative;" :fullscreen="Copyfullscreen" align-center destroy-on-close width="1450px" draggable
            :close-on-click-modal="false">
            <EditTask v-model:id="ids" v-model:copy="typename"></EditTask>
            <el-icon @click="Copyfullscreen = !Copyfullscreen"
                style="position: absolute;right: 45px;top: 24px;cursor: pointer;font-size: 14px;">
                <FullScreen />
            </el-icon>
        </el-dialog>
...
import { View, Delete, Search, Download, Refresh, Upload } from '@element-plus/icons-vue'
import { ElMessage, inputNumberEmits } from 'element-plus'
// const Task = defineAsyncComponent(() => import('./components/completeView.vue'))
const Task = defineAsyncComponent(() => import('./components/viewPage.vue'))
const RedInvoice = defineAsyncComponent(() => import('./components/redInvoice.vue'))
const EditTask = defineAsyncComponent(() => import('./components/HandEdit.vue'))
...
const typename = ref<string>('copy');
// 选中行
...
const CopydialogShow = ref(false);//是否显示窗口
const RedShow = ref(false);//是否显示窗口
const fullscreen = ref(false);//全屏
const fullscreens = ref(false);//全屏
const CopydialogTitle = ref('');//标题
const Copyfullscreen = ref(false);//全屏
...
const maincolumns = ref([
    ...
    { prop: 'buyerName', label: '客户名称', align: 'left', width: '245' },
...
async function getTableData(isQuery) {
    ...
    try {
        ...
        const res = await ajax.post('/Invoice/GetList?srcorgid=' + sobInfo.value.id, qs.stringify(formData.value));
        if (res.data.total == 0) {
...
// 保存
const Save = () => {
    ajax.post('/Invoice/OpenInvoice?srcorgid=' + sobInfo.value.id, qs.stringify({ id: ids.value, invoicedepartname: sellerName.value, taxno: sellerTaxNo.value })).then((res) => {
        if (res.success) {
...
function downLoad() {
    ...
    // utils.showFileAll(name, arrid.value, config.SERVER_WEBADDRESS);
    // ajax.post(config.SERVER_WEBADDRESS + "/Files/ShowAll", qs.stringify({ fileids: arridlist.value, buyernames: name })).then((res) => {
    // })

    // axios.post(config.SERVER_PRO_WEBADDRESS + "/Files/ShowAll", qs.stringify({ fileids: arridlist.value, buyernames: name }), { responseType: 'blob', }, { headers: { "content-type": "multipart/from-data", } }).then((res) => {
...
//     debounce(() => {
//         formref.value.action = config.SERVER_DEV_APIADDRESS + '/Report/ExportInvoiceMain?' + qs.stringify({ whereJSON: JSON.stringify(formData.value) });
...
//     //将选中的数据的eInvoiceUrl用逗号隔开插入到urls里面
//     urls.value = val.map((item) => {
//         return item.eInvoiceUrl;
//     }).join(',');
// }
// 多选
const handleSelectionChange = (val) => {
