    ...
    <div>
        ...
        <div class="form-container">
            <el-form ref="formbutton" inline>
                <el-form-item>
                    <el-button style="width:100px;" type="primary" @click="ticketstart">开票</el-button>
                </el-form-item>
                <el-form-item>
                ...
                <el-table-column label="操作" align="center" width="150" fixed="right">
                    <template #default="scope">
                        ...
                        <el-button type="danger" size="small"
                            @click="DeleteAll(scope.row.id, scope.row.billcode)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <el-dialog v-model="showserch" title="查询" align-center destroy-on-close width="800px" draggable
            ...
            <el-form :inline="true" :model="mainform" class="inline" label-width="100px">
                ...
                <el-form-item label="开票日期">
                    <el-date-picker v-model="mainform.invoiceDate" type="daterange" value-format="YYYY-MM-DD" clearable />
                </el-form-item>
        ...
        <el-dialog v-model="dialogShow" :title="dialogTitle" @close="fullscreen = false" style="position: relative;"
            :fullscreen="fullscreen" align-center destroy-on-close width="1450px" draggable :close-on-click-modal="false">
            <EditTask v-model:id="ids" v-if="!isHand" v-model:serialNo="serialNo" v-model:buyerTaxNo="buyerTaxNo"
                v-model:billcode="billcode" v-model:buyerName="buyerName"></EditTask>
            <HandEditTask v-model:id="ids" v-if="isHand" v-model:serialNo="serialNo" v-model:buyerTaxNo="buyerTaxNo"
                v-model:billcode="billcode" v-model:buyerName="buyerName"></HandEditTask>
...
const HandEditTask = defineAsyncComponent(() => import('./components/HandEditNew.vue'))
...
// const { progress, start } = useRealProgress(dataprovide.value)
...
const maincolumns = ref([
    ...
    { prop: 'remarks', label: '备注', align: 'center', width: '220' },
]);
// 明细表格列
...
async function getTableData() {
    try {
        ...
        mainform.value.date1 = mainform.value.invoiceDate[0]
        mainform.value.date2 = mainform.value.invoiceDate[1]
...
const getsobItems = async () => {
    try {
        ...
        if (res.success) {
            ...
            // sobTaxno.value = res.data[0].taxno;
            // sobItemsName.value = res.data[0].label;
        }
    } catch (error) {
        // 这里可以添加你的错误处理逻辑
    }
...
const openEdit = (row) => {
    ...
    invoiceTypeName.value = row.invoiceTypeName;
    if (row.billcode == '手工开票') {
        isHand.value = true;
    } else {
        isHand.value = false;
    }
