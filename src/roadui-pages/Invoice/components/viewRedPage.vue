...
<template>
    <div class="main-page">
        ...
        <div class="top">
            ...
            <div class="top-div">
                <div style="font-size: 28px;">电子发票（普通发票）</div>
            ...
            <div style="position: absolute; top: 40px; right: 50px;">
                <div style="width: fit-content; text-align: right; color: #AD7353;">发票号码：<span
                        style="font-size: 14px; color: #585859;">{{ formDatas.originalInvoiceNo }}</span></div>
                <div style="width: fit-content; text-align: right; color: #AD7353;">开票日期：<span
                        style="font-size: 14px; color: #585859;">{{ formDatas.originInvoiceDate }}</span></div>
                <div style="width: fit-content; text-align: right; color: #AD7353;">红冲原因：<span
                        style="font-size: 14px; color: red;">{{ redInvoiceLabel }}</span></div>
            </div>
        </div>
                ...
                <tr style="height: 100px !important;">
                    ...
                    <td colspan="3" width="257" style="padding: 15px 10px 0px 10px;">
                        <el-form :model="formDatas" ref="ruleFormRef">
                            ...
                            <el-form-item label="统一社会信用代码/纳税人识别:" prop="buyerTaxNo">
                                <span style="font-size: 12px; color: #585859;">
                    ...
                    <td colspan="3" width="243" style="padding: 15px 10px 0px 10px;">
                        <el-form :model="formDatas" ref="ruleFormRef">
                            ...
                            <el-form-item label="统一社会信用代码/纳税人识别:" prop="sellerTaxNo">
                                <span style="font-size: 12px; color: #585859;">
        ...
        <table border
            ...
            <tr>
                ...
                <td
                    ...
                    <div
                        style="width: calc(100% - 200px); text-align: left; height: 30px; font-size: 12px; color: #585859; line-height: 30px;margin-left: 30px;">
                        {{ useConvertNumMoneyToChinese(formDatas.invoiceTotalPriceTax) }}
                    </div>
                    <div style="line-height: 30px; margin-right: 50px; line-height: 30px">
                        (小写)&nbsp;<span style="font-size: 12px; color: #585859;">￥{{ formDatas.invoiceTotalPriceTax
                        }}</span>
                    </div>
                </td>
            </tr>
...
const redInvoiceLabel = ref('');//红冲标识
...
const getformdata = async () => {
    ...
    try {
        ...
        if (Array.isArray(res.data.invoicemain) && res.data.invoicemain.length > 0) {
            ...
            // 根据formDatas.value.redInvoiceLabel的值显示不同的红冲标识
            if (res.data.invoicemain[0].redInvoiceLabel == '01') {
                redInvoiceLabel.value = '开票有误';
            } else if (res.data.invoicemain[0].redInvoiceLabel == '02') {
                redInvoiceLabel.value = '销货退回';
            } else if (res.data.invoicemain[0].redInvoiceLabel == '03') {
                redInvoiceLabel.value = '服务中止';
            } else if (res.data.invoicemain[0].redInvoiceLabel == '04') {
                redInvoiceLabel.value = '销售折让';
            }
