        ...
        <div>
            <el-form :inline="true">
                ...
                <el-form-item>
                    ...
                    <el-button type="danger" :disabled="butDisabled" @click="delTableData">彻底删除</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div>
            <el-table :data="tableData"
                ...
                <el-table-column prop="senderId" label="来源" width="100" sortable="custom">
                    <template #default="scope">
                        <el-tag effect="dark" v-if="scope.row.type===1">发件箱</el-tag>
                        <el-tag effect="dark" v-else-if="scope.row.type===2" type="success">收件箱</el-tag>
                        <el-tag effect="dark" v-else-if="scope.row.type===3" type="info">草稿箱</el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="mailTime" label="日期" width="180" sortable="custom" />
            </el-table>
        </div>
...
<script setup>
    ...
    const defaultOrder = ref({ prop: 'id', order: 'descending' });
    ...
    const loadTableData = (isQuery) => {
        ...
        ajax.post('/Mail/GetDeletedList', qs.stringify(formData.value)).then((res) => {
    ...
    const delTableData = () => {
        if (selectRows.length === 0) {
            utils.msg('请选择要彻底删除的邮件！', 'error');
            return;
        }
        utils.confirm('您确定要彻底删除所选邮件吗？', () => {
            butDisabled.value = true;
            ajax.post('/Mail/DeleteDeleted', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
                let msg = res.msg;
    ...
    const view = (row) => {
        const item = {
            ...
            url: '/system/mail/view?id=' + row.id + '&source=deleted',
            ico: 'Message',
            openMode: 0,
        };
        index_openmenu(item);
