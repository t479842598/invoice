        ...
        <div style="height:calc(100% - 60px);overflow:auto;">
            <el-scrollbar>
                <el-table :data="tableData"
                    ...
                    <el-table-column prop="ico" label="图标" width="200"  sortable>
                        <template #default="scope">
                           <component v-if="utils.length(scope.row.ico)>0" style="width:18px;height:18px;" :is="scope.row.ico"></component>
                        </template>
                    </el-table-column>
        ...
        <el-dialog v-model="dialogShow" :title="dialogTitle" align-center destroy-on-close width="700px" draggable :close-on-click-modal="false">
            <el-form :model="editData" ref="ruleFormRef" :rules="rules" label-width="80px">
                <el-form-item label="名称" prop="name">
                    <el-input v-model="editData.name" placeholder="按钮显示名称" clearable />
                </el-form-item>
                <el-form-item label="脚本" prop="scripts">
                    <!--<el-input v-model="editData.scripts" :rows="5" placeholder="按钮点击执行的js脚本" type="textarea" clearable />-->
                    <el-codemirror v-model="editData.scripts" placeholder="按钮点击执行的js脚本" height="120px"></el-codemirror>
                </el-form-item>
                ...
                <el-form-item label="操作地址" prop="address">
                    <el-input v-model="editData.address" placeholder="按钮对应的后端操作地址，验证权限用。"></el-input>
                </el-form-item>
                <el-form-item label="备注" prop="note">
                    <el-input v-model="editData.note" placeholder="备注说明"></el-input>
                </el-form-item>
...
<script setup>
    ...
    const rules = {
        ...
        scripts: [{ required: true, message: '请输入按钮脚本', trigger: 'blur' }],
    };
    ...
    const loadTableData = () => {
        ...
        ajax.post('/Button/GetList', qs.stringify({ findname: formData.value.name })).then((res) => {
    ...
    const edit = (row) => {
        if (utils.isUndef(row) || utils.length(row.id) === 0) {
            editData.value = { id: utils.emptyLong, name: '', type: 'primary', sort: utils.getArrayMax(tableData.value, 'sort') + 5 };
            dialogTitle.value = '添加';
            dialogShow.value = true;
            return;
        }
        if (utils.length(row.id) > 0) {
            ajax.post('/Button/Get?buttonid=' + row.id).then((res) => {
                if (res.success) {
    ...
    const save = async (formEl) => {
        ...
        await formEl.validate((valid) => {
            if (valid) {
                ...
                ajax.post('/Button/Save?buttonid=' + editId, qs.stringify(editData.value)).then((res) => {
                    let msg = res.msg;
    ...
    const remove = () => {
        if (selectRows.length === 0) {
            utils.msg('请选择要删除的按钮！', 'error');
            return;
        }
        utils.confirm('您确定要删除所选按钮吗？', () => {
            butDisabled.value = true;
            ajax.post('/Button/Delete', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
