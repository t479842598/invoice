            ...
            <el-table :data="tableData"
                ...
                <el-table-column prop="contents" label="消息内容" sortable="custom">
                    <template #default="scope">
                        <a class="roaduia" href="javascript:;" v-if="utils.length(scope.row.url)>0" @click="openUrl(scope.row)">{{scope.row.contents}}</a>
                        <span v-else>{{scope.row.contents}}</span>
                    </template>
                </el-table-column>
...
<script setup>
    ...
    const loadTableData = (isQuery) => {
        ...
        ajax.post('/Message/GetReadList', qs.stringify(formData.value)).then((res) => {
    ...
    const del = () => {
        ...
        utils.confirm('您确定要删除所选消息吗？', () => {
            butDisabled.value = true;
            ajax.post('Message/Delete?type=1', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
                let msg = res.msg;
