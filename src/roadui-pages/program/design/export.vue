<template>
    <div>
        <el-table :data="tableData"
            ...
            <el-table-column prop="showType" label="显示方式" width="160">
            ...
            <!--<el-table-column prop="width" label="宽度" width="80">
                <template #default="scope">
                    {{scope.row.width==0?'':scope.row.width}}
                </template>
            </el-table-column>-->
        ...
        <div style="margin:15px auto 0 auto; text-align:center;">
            <el-button :disabled="butDisabled" type="primary" @click="copyList()">复制列表字段</el-button>
        ...
        <el-dialog v-model="dialogShow" :title="dialogTitle" align-center destroy-on-close width="650px" draggable :close-on-click-modal="false">
            <el-form :model="editData" ref="ruleFormRef" :rules="rules" label-width="80px">
                ...
                <el-form-item label="数据类型" prop="dataType">
                    <el-select v-model="editData.dataType" style="width:210px;" clearable>
                        <el-option v-for="op in dataTypeOptions" :key="op.value" :value="op.value" :label="op.label"></el-option>
                    </el-select>
                    <span style="margin-left: 12px" v-show="editData.dataType==2||editData.dataType==3">
                        格式<el-input v-model="editData.showFormat" style="width: 260px; margin-left: 12px" clearable />
                    </span>
                </el-form-item>
                <el-form-item label="显示方式" prop="showType">
                    ...
                    <span style="margin-left:12px">
                        对齐
                        <el-select v-model="editData.align" style="width: 120px; margin-left: 12px" clearable>
                ...
                <el-form-item label="显示顺序" prop="sort">
                    ...
                    <span v-if="editData.showType==10" style="margin-left:12px;">
                        自定义<el-input v-model="editData.customString" style="width:354px;margin-left:12px;" clearable placeholder="自定义字符串" />
                    </span>
                </el-form-item>
                <el-form-item>
...
<script setup>
    ...
    const dataTypeOptions = ref([]);//数据类型选项
    ...
    const loadOptions = () => {
        const params = [
            ...
            { id: 'programdesigner_exportdatatype', key: 'dataType', children: false, valueField: 'value' },
        ];
        ajax.post('/Dictionary/GetMultipleOptions', qs.stringify({ json: JSON.stringify(params) })).then((res) => {
            ...
            dataTypeOptions.value = res.data['dataType'];
            //去掉按钮列选项
            for (let i = 0; i < showTypeOptions.value.length; i++) {
                if (showTypeOptions.value[i].value == '100') {
                    showTypeOptions.value.splice(i, 1);
                    break;
                }
            }
            loadTableData(false);
    ...
    const loadTableData = (isQuery) => {
        ...
        ajax.post('/Program/GetExportList?programid=' + programId).then((res) => {
    ...
    const edit1 = (row) => {
        if (utils.isDef(row)) {
            ajax.post('/Program/GetExport?fieldid=' + row.id).then((res) => {
                editData.value = res.data;
                editData.value.showType = editData.value.showType + '';
                editData.value.dataType = editData.value.dataType + '';
                editData.value.width = editData.value.width <= 0 ? '' : editData.value.width;
                dialogTitle.value = '编辑 - ' + res.data.field;
                dialogShow.value = true;
            }).catch(() => { });
        } else {
            editData.value = { programId: programId, showType: '0', dataType: '0', align: 'left', sort: utils.getArrayMax(tableData.value, 'sort') + 5 };
    ...
    const save = async (formEl) => {
        ...
        await formEl.validate((valid) => {
            if (valid) {
                butDisabled.value = true;
                ajax.post('/Program/SaveExport', qs.stringify(editData.value)).then((res) => {
                    let msg = res.msg;
    ...
    const del = () => {
        if (selectRows.length === 0) {
            utils.msg('您没有选择要删除的导出字段！', false);
            return;
        }
        utils.confirm('您确定要删除所选导出字段吗？', () => {
            butDisabled.value = true;
            ajax.post('/Program/DeleteExport', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
                butDisabled.value = false;
    ...
    //复制列表字段
    const copyList = () => {
        utils.confirm('复制将清除现有导出字段设置，确定要复制吗？', () => {
            butDisabled.value = true;
            ajax.post('/Program/CopyListToExport?programid=' + programId).then((res) => {
                butDisabled.value = false;
                let msg = '';
                switch (res.code) {
                    case 0:
