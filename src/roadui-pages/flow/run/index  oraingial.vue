...
const regFormComponent = (data) => {
    ...
    const component = {
        ...
        computed: {
            ...
            //得到下拉选择只读时标题 -- row:子表时的行数据，有时下拉联动需要用row的key来取当前行的值。
            getSelectTitle() {
                return function (id, value, row) {
                    const options = utils.isDef(row) && utils.hasKey(row, 'key') && this.formSelectOptions[id + '_' + row['key']]
                        ? this.formSelectOptions[id + '_' + row['key']]
                        : this.formSelectOptions[id];
        ...
        methods: {
            ...
            addSubtableRow(id, record) {
                ...
                if (utils.isDef(defaultValue)) {
                    ...
                    //执行添加事件
                    this.execFormEvent('subtable_event_' + id, 'addRow', defaultValue1.key, defaultValue1);
                }
            },
            //子表插入行
            ...
            delSubtableRow(id, record) {
                ...
                //执行删除事件
                this.execFormEvent('subtable_event_' + id, 'deleteRow', record.key, record);
            },
            //子表计算合计
...
const executeFlow = (type, steps) => {
    ...
    ajax.post('/Flow/FlowExecute?srcorgid=' + sobInfo.value.id + queryString, qs.stringify({ json: JSON.stringify(json), formData: JSON.stringify(form.value.formData) })).then((res) => {
        if (res.success) {
...
//处理过程
const process = (isSubflow) => {
    dialogQuery.value = queryString + (isSubflow ? '&issubflow=1' : '');
