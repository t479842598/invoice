    ...
    <div v-loading="loading" :element-loading-text="loadingText">
        ...
        <div :style="{ 'height': formHeight, 'overflow': 'auto' }">
            ...
            <div>
                ...
                <div style="padding-top:12px;">
                    <component ref="roadComponentFormRef" v-if="form.type === 0 || form.type === 1" :is="formComponentId"
                        :query="formComponentQuery"></component>
                    <iframe v-else :src="form.address"
                        :style="`border:none;width:100%;height:${parseInt(formHeight) - 20}px;`" frameborder="0"></iframe>
                </div>
                <!--历史意见列表-->
        ...
        <div v-if="'1' == isMobile" class="roadui_noprint" style="border-top: 1px solid var(--el-border-color-light);">
            <div v-if="step.signType > 0 && (step.display == 0 || step.display == 4)"
                ...
                <el-select placeholder="常用" @change="(val) => { step.comment = val; }" style="width:75px;margin-right:8px;">
                    <el-option v-for="(comment, index) in form.commentList" :key="index" :label="comment"
                        :value="comment"></el-option>
                </el-select>
...
//     var nroadflowtoken = JSON.parse(e.token);
//     var nroadflowclientid = JSON.parse(e.clientid);
//     // 存储数据到H5的localStorage
//     localStorage.setItem('nroadflow-token', JSON.stringify(nroadflowtoken));
//     localStorage.setItem('nroadflow-clientid', JSON.stringify(nroadflowclientid));
//     console.log('nroadflow-token', nroadflowtoken)
//     console.log('nroadflow-clientid', nroadflowclientid)

// }, false);
// 弹窗
const id = ref('')
id.value = localStorage.getItem('formid')
onMounted(() => {
    nextTick(() => {
        loadRun();
    });
        ...
        methods: {
            ...
            linkageAddChilds(id, linkageId, linkageDataSource, linkageText, sqlConnId, val, row) {
                ...
                ajax.post('/Form/GetSelectChildOptions', qs.stringify({
                    value: utils.aes(value),//上级值
                    linkageDataSource: linkageDataSource,//数据源类型 dict,sql,string
                    linkageText: utils.aes(linkageText), //数据源如为SQL的sql语句
...
const executeFlow = (type, steps) => {
    ...
    if (task == null) {
        ...
        id.value = task.formid
        formtype.value = task.formtype
    }
    ...
    ajax.post('/Flow/FlowExecute?srcorgid=' + sobInfo.value.id + queryString, qs.stringify({ id: id.value, formtype: formtype.value, json: JSON.stringify(json), formData: JSON.stringify(form.value.formData) })).then((res) => {
        if (res.success) {
            emits("index", false);
...
const executeFlows = (type, steps) => {
    if (task.taskType == '4' && task.stepId == FristStepId.value) {
        istj.value = '1'
    } else {
        istj.value = '0'
    }
    ...
    ajax.post('/Flow/FlowExecute?srcorgid=' + sobInfo.value.id + queryString, qs.stringify({
        id: id.value
        , istj: istj.value
        , isapply: '1'
        ...
        if (res.success) {
            index_refreshtab();
            nextTick(() => {
                roadComponentFormRef.value.removecontent()
            })
            const resData = res.data;
            //消息提示
            if (utils.length(programId) > 0) {
                ...
                utils.alert(getExecuteMsg(resData, type), '', '', resData.code == 0, () => {
                    // executeAfter(resData, type, json);
                    index_removetab();
                    index_refreshtab();
                });
        ...
        butDisabled.value = false;
        loading.value = false;
        // localStorage.removeItem('projectJSON')
        // localStorage.removeItem('departperformancetargetJSON')
        // localStorage.removeItem('measurementJSON')
        // localStorage.removeItem('projectequipmentJSON')
        // localStorage.removeItem('attachmentJSON')
    });
};
//保存
const save = async () => {
    nextTick(() => {
        roadComponentFormRef.value.savecontent()
    });
...
//确定发送
const sendOk = (steps) => {
    executeFlows('Submit', steps);
};
// --------------------
//退回
const back = () => {
    formData.value.formId = localStorage.getItem('formid')
    //检查如果要签意见，是否签意见和签章
    if (step.value.signType > 0 && utils.length(step.value.comment) === 0) {
        utils.msg('请填写处理意见！', false);
        return;
...
const dele = () => {
    ...
    ElMessageBox.confirm(
        ...
        ajax.post('/Flow/FlowExecute?srcorgid=' + sobInfo.value.id, qs.stringify({ id: tasks.formid, formtype: tasks.formtype, json: JSON.stringify(json.value) })).then((res) => {
            if (res.data.code == 0) {
                //重新加载数据
                emits("index", false);
                //消息提示
