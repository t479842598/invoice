            ...
            <pane :size="84" style="background: #fff; overflow: auto; padding-left: 5px;">
                ...
                <div>
                    <el-table :data="tableData"
                        ...
                        <el-table-column prop="name" label="流程名称" show-overflow-tooltip sortable="custom" />
                        <el-table-column prop="createDate" label="创建时间" width="180" sortable="custom" />
                        <el-table-column prop="createUser" label="创建人" width="140" sortable="custom" />
...
<script setup>
    ...
    const index_changetab = inject('index_changetab');//首页切换tab
    ...
    const loadTreeNode = (node, resolve) => {
        //如果是首次加载
        if (node.level === 0) {
            ajax.post('/Dictionary/GetTreeJson?root=system_applibrary_flowtype').then((res) => {
                //添加一个已删除节点
                const deleteNode = {
                    id: utils.emptyLong + '',
                    parentId: '0',
    ...
    const defaultOrder = ref({ prop: 'createdate', order: 'ascending' });
    const pager = ref(utils.getPager(defaultOrder.value));

    //得到状态标题
    const getStatusTitle = computed(() => (status) => {
        return utils.getFlowStatusTitle(status);
    });
    //得到状态标签颜色
    ...
    const loadTableData = (isQuery) => {
        ...
        ajax.post('/Flow/GetList', qs.stringify(formData.value)).then((res) => {
    ...
    const edit = (row) => {
        ...
        if (tabIndex >= 0) {
            ...
            index_changetab(index_tabs.value[tabIndex].id);
            return;
        }
        const menu = {
            id: 'flowdesign_' + (isNew ? utils.emptyLong + '' : row.id),
            title: isNew ? '新建流程' : '设计流程-' + row.name,
            ico: 'Edit',
            openMode: 0,
            url: '/flow/design/design?isnew=' + (isNew ? '1' : '0') + '&typeid=' + formData.value.type + '&flowid=' + (isNew ? '' : row.id),
        };
        index_openmenu(menu);
    };
    //删除
    const remove = () => {
        if (selectRows.length === 0) {
            utils.msg('您没有选择要删除的流程！', false);
            return;
        }
        utils.confirm('您确定要删除所选流程吗？', () => {
            butDisabled.value = true;
            ajax.post('/Flow/DeleteFlow', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
    ...
    const thoroughRemove = () => {
        if (selectRows.length === 0) {
            utils.msg('您没有选择要彻底删除的流程！', false);
            return;
        }
        utils.confirm('您确定要彻底删除所选流程吗？', () => {
            butDisabled.value = true;
            ajax.post('/Flow/ThoroughDeleteFlow', qs.stringify({ ids: utils.getArrayValues(selectRows, 'id').join(',') })).then((res) => {
                if (res.success) {
                    utils.msg('彻底删除成功！', true);
                    loadTableData(false);
                } else {
                    if (res.code == 1) {
                        utils.msg(res.data + '已有实例任务，不能彻底删除！', false);
                    } else {
                        utils.msg('彻底删除失败！', false);
                    }
                    selectRows = [];
                    loadTableData(false);
                }
    ...
    //恢复
    const recovery = (row) => {
        utils.confirm('您确定要恢复该流程吗？', () => {
            butDisabled.value = true;
            ajax.post('/Flow/RecoveryFlow', qs.stringify({ id: row.id })).then((res) => {
                if (res.success) {
                    utils.msg('恢复成功！', true);
                    loadTableData(false);
                } else {
    ...
    const export1 = () => {
        ...
        if (selectIds.length === 0) {
            utils.msg('您没有选择要导出的流程！', false);
            return;
        }
        const url = config.SERVER_WEBADDRESS + '/Export/FlowExport?flowid=' + selectIds.join(',');
    ...
    const handleAvatarSuccess = (response, uploadFile) => {
        if (uploadFile.status === 'success') {
            ...
            ajax.post('/Flow/ImportFlow', qs.stringify({ files: fileId })).then((res) => {
                let msg = res.msg;
