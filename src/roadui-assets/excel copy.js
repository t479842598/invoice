...
export function excelImport(file, outtype = "json") {
    ...
    return new Promise((resolve, reject) => {
        reader.onloadend = function (e) {
            ...
            // 新增部分：移除A1到J1和A2到J2的数据
            for (let i = 0; i <= 9; i++) { // A1到J1
                delete ws[XLSX.utils.encode_cell({ r: 0, c: i })];
            }
            for (let i = 0; i <= 9; i++) { // A2到J2
                delete ws[XLSX.utils.encode_cell({ r: 1, c: i })];
            }
            for (let i = 0; i <= 9; i++) { // A2到J2
                delete ws[XLSX.utils.encode_cell({ r: 2, c: i })];
            }

            let result = "";
            switch (outtype.toLowerCase()) {
                case "html":
                    result = XLSX.utils.sheet_to_html(ws);
                    break;
                case "json":
                    result = XLSX.utils.sheet_to_json(ws);
                    break;
                default:
                    ElMessage.error('输出类型错误，只能取值为 html 或者 json');
            }
            resolve(result);
        };
